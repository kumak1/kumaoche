...
class TestGit(unittest.TestCase):
    def setUp(self):
        ...
        self.empty_config = StubConfig.find('variable_assign_test_role')
    ...
    def test_run(self):
        self.assertEqual(f"work_dir:repo {self.dst_text},command:run test_command", self.git.run("test_command"))
        self.assertEqual(f"work_dir:repo {self.dst_text},command:run {self.dst_text}", self.git.run(self.src_text))
